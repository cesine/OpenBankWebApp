 ...
 *
 */
if (!window.Zapatec || (Zapatec && !Zapatec.include)) { alert("You need to include zapatec.js file!"); } else { Zapatec.calendarPath = Zapatec.getPath("Zapatec.CalendarWidget"); } // initialize the preferences object; // embed it in a try/catch so we don't have any surprises try { Zapatec.Calendar.loadPrefs(); } catch(e) {};

Zapatec.Calendar=function(oArg,date,onSelect,onClose){if(typeof oArg!='object'){var firstDay=oArg;oArg={};if(firstDay!=null)oArg.firstDay=firstDay;if(date)oArg.date=date;if(onSelect)oArg.onSelect=onSelect;if(onClose)oArg.onClose=onClose;}
Zapatec.Calendar.SUPERconstructor.call(this,oArg);};Zapatec.Calendar.activeCalendar=null;Zapatec.Calendar.id="Zapatec.Calendar";Zapatec.inherit(Zapatec.Calendar,Zapatec.Widget);Zapatec.Calendar.prototype.init=function(oArg){Zapatec.Calendar.SUPERclass.init.call(this,oArg);var oConfig=this.config;this.activeDiv=null;this.historyDateFormat=oConfig.ifFormat||oConfig.daFormat||'%B %d, %Y';this.currentDateEl=null;this.getDateStatus=null;this.getDateToolTip=null;this.timeout=null;this.dragging=false;this.hidden=false;this.minYear=1970;this.maxYear=2050;this.minMonth=0;this.maxMonth=11;this.isPopup=false;this.hiliteToday=true;this.table=null;this.element=null;this.tbody=[];this.firstDayName=null;this.monthsCombo=null;this.hilitedMonth=null;this.activeMonth=null;this.yearsCombo=null;this.hilitedYear=null;this.activeYear=null;this.histCombo=null;this.hilitedHist=null;this.dateClicked=false;this.titles=[];this.rowsOfDayNames=[];this.lastRowHilite=null;this.closeButton=null;this.helpButton=null;this.triggerEl=null;if(!this.isCreate)
...
this.isPopup=true;if(typeof oConfig.onSelect=="function")
this.onSelected=oConfig.onSelect;else
this.onSelected=this.onSelectInner;if(typeof oConfig.onClose!="function")
...
if(oConfig.canType){Zapatec.Utils.addEvent(oConfig.inputField,"mousedown",Zapatec.Calendar.cancelBubble);Zapatec.Utils.addEvent(oConfig.inputField,"keydown",Zapatec.Calendar.cancelBubble);Zapatec.Utils.addEvent(oConfig.inputField,"keypress",Zapatec.Calendar.cancelBubble);Zapatec.Utils.addEvent(oConfig.inputField,"keyup",function(ev){var cal=Zapatec.Calendar.activeCalendar;var format=oConfig.inputField?oConfig.ifFormat:oConfig.daFormat;var parsedDate=Zapatec.Date.parseDate(oConfig.inputField.value,format);if(parsedDate&&!cal.hidden){if(cal.setDate(parsedDate)){cal.showHint(Zapatec.Calendar.i18n("SEL_DATE",null,cal));}}});Zapatec.Utils.addEvent(oConfig.inputField,"blur",function(ev){var cal=Zapatec.Calendar.activeCalendar;var format=oConfig.inputField?oConfig.ifFormat:oConfig.daFormat;var parsedDate=Zapatec.Date.parseDate(oConfig.inputField.value,format);if(!parsedDate){oConfig.inputField.value=cal.printWith2Time(cal.currentDate,cal.currentDateEnd,format);}});}
...
this.show();}else if(oConfig.showAfterCreation){this.unHide(triggerEl);}};Zapatec.Calendar.prototype.reconfigure=function(oArg){Zapatec.Calendar.SUPERclass.reconfigure.call(this,oArg);this.show();};Zapatec.Calendar.prototype.configure=function(oArg){this.defineConfigOption('inputField',null);this.defineConfigOption('displayArea',null);this.defineConfigOption('button',null);this.defineConfigOption('eventName','click');this.defineConfigOption('closeEventName',null);this.defineConfigOption('ifFormat','%Y/%m/%d');this.defineConfigOption('daFormat','%Y/%m/%d');this.defineConfigOption('singleClick',true);this.defineConfigOption('disableFunc',null);this.defineConfigOption('dateStatusFunc',oArg.disableFunc?oArg.disableFunc:null);this.defineConfigOption('dateText',null);this.defineConfigOption('firstDay',null);this.defineConfigOption('align','Br');this.defineConfigOption('range',[1900,2999]);this.defineConfigOption('weekNumbers',true);this.defineConfigOption('flat',null);this.defineConfigOption('flatCallback',null);this.defineConfigOption('onSelect',null);this.defineConfigOption('onClose',null);this.defineConfigOption('onUpdate',null);this.defineConfigOption('onFDOW',null);this.defineConfigOption('noGrab',false);this.defineConfigOption('date','');this.defineConfigOption('dateEnd','');this.defineConfigOption('showsTime',false);this.defineConfigOption('sortOrder','asc');this.defineConfigOption('timeFormat','24');this.defineConfigOption('timeInterval',null);this.defineConfigOption('electric',true);this.defineConfigOption('step',2);this.defineConfigOption('position',null);this.defineConfigOption('cache',true);this.defineConfigOption('showOthers',false);this.defineConfigOption('multiple',null);this.defineConfigOption('multipleRange',null);this.defineConfigOption('multipleSelection',false);this.defineConfigOption('saveDate',null);this.defineConfigOption('fdowClick',false);this.defineConfigOption('titleHtml',null);this.defineConfigOption('noHelp',false);this.defineConfigOption('noCloseButton',false);this.defineConfigOption('disableYearNav',false);this.defineConfigOption('disableFdowChange',false);this.defineConfigOption('multiple',null);this.defineConfigOption('disableDrag',false);this.defineConfigOption('numberMonths',1);this.defineConfigOption('monthsInRow',oArg.numberMonths?oArg.numberMonths:1);this.defineConfigOption('controlMonth',1);this.defineConfigOption('vertical',false);this.defineConfigOption('canType',false);this.defineConfigOption('theme','bluexp');this.defineConfigOption('themeSize','');if(Zapatec.Langs){this.defineConfigOption('langId',Zapatec.Calendar.id);this.defineConfigOption('lang','en');}
this.defineConfigOption('showEffect','');this.defineConfigOption('showEffectSpeed',100);this.defineConfigOption('showEffectOnFinish',null);this.defineConfigOption('hideEffect','');this.defineConfigOption('hideEffectSpeed',100);this.defineConfigOption('hideEffectOnFinish',null);this.defineConfigOption('showAfterCreation',false);this.defineConfigOption('noHistory',false);this.defineConfigOption('noStatus',false);this.defineConfigOption('minimal',false);this.defineConfigOption('maxSelection',-1);this.defineConfigOption('onWeekClick',null);this.defineConfigOption('onTodayClick',null);this.defineConfigOption('onMonthSelect',null);this.defineConfigOption('onYearSelect',null);this.defineConfigOption('onHistorySelect',null);this.defineConfigOption('setDateToolTip',null);this.defineConfigOption('timeRange',false);this.defineConfigOption('onCreate',null);this.defineConfigOption('closeButton',null);this.defineConfigOption('helpButton',null);Zapatec.Calendar.SUPERclass.configure.call(this,oArg);var oConfig=this.config;if(!oConfig.prefs)
oConfig.prefs={fdow:null,history:"",hsize:9};oConfig.themeSize=oConfig.themeSize.toLowerCase();if(oConfig.themeSize=='normal')
oConfig.themeSize='';var firstLetter=oConfig.themeSize.substr(0,1);oConfig.themeSize=firstLetter.toUpperCase()+oConfig.themeSize.substr(1);if(oConfig.prefs.fdow||(oConfig.prefs.fdow==0)){oConfig.firstDay=parseInt(oConfig.prefs.fdow,10);}else{var fd=0;if(typeof oConfig.firstDay=="number"){fd=oConfig.firstDay;}else if(typeof this._FD=='number'){fd=this._FD;}
oConfig.firstDay=fd;}
if(oConfig.weekNumbers){oConfig.disableFdowChange=true;oConfig.firstDay=1;}else{oConfig.onWeekClick=null;}
if((oConfig.numberMonths>12)||(oConfig.numberMonths<1)){oConfig.numberMonths=1;}
if(oConfig.numberMonths>1){oConfig.showOthers=false;}
oConfig.numberMonths=parseInt(oConfig.numberMonths,10);if((oConfig.controlMonth>oConfig.numberMonths)||(oConfig.controlMonth<1)){oConfig.controlMonth=1;}
...
if(oConfig.noHelp||!Zapatec.Button){oConfig.helpButton=null;}};Zapatec.Calendar.prototype.unHide=function(triggerEl){var cal=this;cal.lastRowHilite=null;if(!cal)
...
if(dateValue!=""){var parsedDate=Zapatec.Date.parseDate(dateEl.value||dateEl.innerHTML,dateFmt);if(parsedDate!=null){if(!(typeof cal.config.dateStatusFunc=="function"&&cal.config.dateStatusFunc(parsedDate))&&!cal.setDate(parsedDate))
...
var update=(cal.dateClicked||p.electric);if(update&&p.flat){if(typeof p.flatCallback=="function")
{if(!p.multiple)
...
return Zapatec.Calendar.i18nOld(str,type);};Zapatec.Calendar.savePrefs=function(calendarId){var cal=Zapatec.Widget.getWidgetById(calendarId);Zapatec.Utils.writeCookie("ZP_CAL",Zapatec.Utils.makePref(cal.config.prefs),null,'/',30);};Zapatec.Calendar.loadPrefs=function(){var txt=Zapatec.Utils.getCookie("ZP_CAL"),tmp;if(txt){tmp=Zapatec.Utils.loadPref(txt);if(tmp){Zapatec.Utils.mergeObjects(this.config.prefs,tmp);}}};Zapatec.Calendar._add_evs=function(el){var C=Zapatec.Calendar;el.onmouseover=C.dayMouseOver;el.onmousedown=C.dayMouseDown;el.onmouseout=C.dayMouseOut;if(Zapatec.is_ie){el.ondblclick=C.dayMouseDblClick;}};Zapatec.Calendar._del_evs=function(el){if(!el){return null;}
...
s.top=(cd.offsetTop+cd.offsetHeight)+"px";cal.updateWCH(mc);};Zapatec.Calendar.showHistoryCombo=function(){var cal=Zapatec.Calendar.activeCalendar;var a,h,i,cd,hc,s,tmp,div;if(!cal)
return false;hc=cal.histCombo;while(hc.firstChild)
hc.removeChild(hc.lastChild);if(cal.config.prefs.history){a=cal.config.prefs.history.split(/,/);i=0;while(tmp=a[i++]){tmp=tmp.split(/\//);h=Zapatec.Utils.createElement("div");h.className=Zapatec.is_ie?"label-IEfix":"label";h.id="zpCal"+cal.id+"HistoryDropdownItem"+(i-1);h.histDate=new Date(parseInt(tmp[0],10),parseInt(tmp[1],10)-1,parseInt(tmp[2],10),tmp[3]?parseInt(tmp[3],10):0,tmp[4]?parseInt(tmp[4],10):0);h.appendChild(window.document.createTextNode(Zapatec.Date.print(h.histDate,cal.historyDateFormat)));hc.appendChild(h);if(Zapatec.Date.dateEqualsTo(h.histDate,cal.config.date))
Zapatec.Utils.addClass(h,"active");}}
cd=cal.activeDiv;s=hc.style;s.display="block";s.left=Math.floor(cd.offsetLeft+(cd.offsetWidth-hc.offsetWidth)/2)+"px";s.top=(cd.offsetTop+cd.offsetHeight)+"px";cal.updateWCH(hc);cal.bEventShowHistory=true;};Zapatec.Calendar.showYearsCombo=function(fwd){var cal=Zapatec.Calendar.activeCalendar;if(!cal){return false;}
...
window.document.selection.empty();};Zapatec.Calendar.dayMouseOver=function(ev){var el=Zapatec.Utils.getElement(ev);while(!el.calendar){el=el.parentNode;caldate=el.caldate;}
...
if(el.ttip){if(el.ttip.substr(0,1)=="_"){el.ttip=Zapatec.Date.print(caldate,el.calendar.ttDateFormat)+el.ttip.substr(1);}
...
newdate=true;if(el.otherMonth){cal._init(cal.config.firstDay,date);}
cal.onSetTime();}else{if(el.navtype==200){if(!cal.closeButton){Zapatec.Utils.removeClass(el,"hilite");cal.callCloseHandler();}
return;}
...
break;case-1:if(mon>0){setMonth(mon-cal.config.numberMonths);}else if(year-->cal.minYear){Zapatec.Date.setFullYear(date,year);setMonth(11);}
if(typeof cal.config.onMonthSelect=="function"){cal.config.onMonthSelect(mon>0?mon-1:11);}
break;case 1:if(mon<11){setMonth(mon+cal.config.numberMonths);}else if(year<cal.maxYear){Zapatec.Date.setFullYear(date,year+1);setMonth(0);}
...
break;}
if(!Zapatec.Date.equalsTo(date,cal.config.date)){if((el.navtype>=-2&&el.navtype<=2)&&(el.navtype!=0)){cal._init(cal.config.firstDay,date,true);return;}
...
cal.config.onYearSelect(year.year);}else{var hist=Zapatec.Calendar.findHist(target);if(hist&&!Zapatec.Date.dateEqualsTo(hist.histDate,cal.config.date)){date=new Date(hist.histDate);cal._init(cal.config.firstDay,cal.config.date=date);cal.dateClicked=false;cal.callHandler();if(!cal.config.noHistory)
cal.updateHistory();if(typeof cal.config.onHistorySelect=="function")
...
Zapatec.Calendar.prototype.create=function(_par){var parent=null;if(!_par){parent=window.document.getElementsByTagName("body")[0];this.isPopup=true;this.WCH=Zapatec.Utils.createWCH();}else{parent=_par;this.isPopup=false;}
this.currentDate=this.config.date=this.dateStr?new Date(this.dateStr):new Date();if(this.config.timeRange)
this.currentDateEnd=new Date(this.currentDate);var table=Zapatec.Utils.createElement("table");table.cellSpacing=0;table.cellPadding=0;this.table=table;Zapatec.Utils.createProperty(table,"calendar",this);Zapatec.Utils.addEvent(table,"mousedown",Zapatec.Calendar.tableMouseDown);if(Zapatec.is_opera){table.style.width=(this.config.monthsInRow*((this.config.weekNumbers)?(8):(7))*2+4.4*this.config.monthsInRow)+"em";}
var div=Zapatec.Utils.createElement("div");this.element=div;this.container=this.element;var saveDiv=div;div.className="calendar "+this.getClassName({prefix:"zpCalendar"})+" zpCalendar"+this.config.themeSize;div.id="zpCal"+this.id+"Container";if(Zapatec.is_ie){Zapatec.Utils.addEvent(div,"mousewheel",new Function("Zapatec.Calendar.zoomElement(window.event,'"+div.id+"')"));}
...
cell.colSpan=colspan;cell.className="button";cell.innerHTML="<div>&nbsp</div>";return cell;};var title_length=((this.config.weekNumbers)?(8):(7))*this.config.monthsInRow-2;var thead=Zapatec.Utils.createElement("thead",table);if(this.config.numberMonths==1){this.title=thead;}
...
if(this.config.numberMonths!=1){row=Zapatec.Utils.createElement("tr",thead);var title_length=5;this.config.weekNumbers&&++title_length;this.titles[l]=new Array();for(var k=1;(k<=this.config.monthsInRow)&&((l-1)*this.config.monthsInRow+k<=this.config.numberMonths);++k){hd(row,1);this.titles[l][k]=hh("&nbsp;",title_length,300);this.titles[l][k].className="title";this.titles[l][k].id="zpCal"+this.id+"SubTitle"+((l-1)*this.config.monthsInRow+k);hd(row,1);}}
...
return Zapatec.Utils.stopEvent(ev);};Zapatec.Calendar.prototype._init=function(firstDay,date,last){var
...
if(validMonth<0){Zapatec.Date.setFullYear(tmpDate,tmpDate.getFullYear()-1);validMonth=12+validMonth;}
if(validMonth>11){Zapatec.Date.setFullYear(tmpDate,tmpDate.getFullYear()+1);validMonth=validMonth-12;}
validMonths[l][k]=validMonth;tmpDate.setDate(1);tmpDate.setMonth(validMonth);day1=(tmpDate.getDay()-this.config.firstDay)%7;if(day1<0)
day1+=7;hrs=tmpDate.getHours();tmpDate.setDate(-day1);tmpDate.setDate(tmpDate.getDate()+1);if(hrs!=tmpDate.getHours()){tmpDate.setDate(1);tmpDate.setMonth(validMonth);tmpDate.setDate(-day1);tmpDate.setDate(tmpDate.getDate()+1);}
months[l][k]=tmpDate;}}
var MN=Zapatec.Calendar.i18n(month,"smn",this);var weekend=Zapatec.Calendar.i18n("WEEKEND",null,this);var dates=this.config.multiple?(this.datesCells={}):null;var DATETXT=this.config.dateText;var row,i,k,cell,hasdays,iday,wday,dmonth,dyear,current_month;for(var l=1;l<=rowsOfMonths;++l){row=this.tbody[l].firstChild;for(i=7;--i>0;row=row.nextSibling){cell=row.firstChild;hasdays=false;for(k=1;(k<=this.config.monthsInRow)&&((l-1)*this.config.monthsInRow+k<=this.config.numberMonths);++k){date=months[l][k];if(this.config.weekNumbers){cell.className="day wn";cell.innerHTML=Zapatec.Date.getWeekNumber(date);if(k>1){cell.className+=" month-left-border";}
cell=cell.nextSibling;}
row.className="daysrow";row.id="zpCal"+this.id+"Daysrow"+(6-i);for(j=7;cell&&(iday=date.getDate())&&(j>0);date.setDate(iday+1),((date.getDate()==iday)?(date.setHours(1)&&date.setDate(iday+1)):(false)),cell=cell.nextSibling,--j){wday=date.getDay(),dmonth=date.getMonth(),dyear=date.getFullYear();cell.className="day";if((!this.config.weekNumbers)&&(j==7)&&(k!=1)){cell.className+=" month-left-border";}
if((j==1)&&(k!=this.config.monthsInRow)){cell.className+=" month-right-border";}
current_month=!(cell.otherMonth=!(dmonth==validMonths[l][k]));if(!current_month){if(this.config.showOthers)
cell.className+=" othermonth";else{cell.className+=" true";cell.innerHTML="&nbsp;";continue;}}else
hasdays=true;cell.innerHTML=DATETXT?DATETXT(date,dyear,dmonth,iday):iday;dates&&(dates[Zapatec.Date.print(date,"%Y%m%d")]=cell);if(!cell.disabled){cell.caldate=[dyear,dmonth,iday];cell.ttip="_";if((weekend!=null)&&(weekend.indexOf(wday.toString())!=-1)){cell.className+=cell.otherMonth?" oweekend":" weekend";}
if(dyear==TY&&dmonth==TM&&iday==TD){cell.className+=" today";cell.ttip+=Zapatec.Calendar.i18n("PART_TODAY",null,this);}
...
if(this.getDateStatus){status=this.getDateStatus(date,dyear,dmonth,iday);if(this.getDateToolTip){toolTip=this.getDateToolTip(date,dyear,dmonth,iday);if(toolTip)
cell.title=toolTip;}
if(status==true){cell.className+=" disabled";}else{cell.className+=" "+status;}}}
if(!(hasdays||this.config.showOthers))
...
if(this.config.numberMonths==1){this.title.innerHTML=Zapatec.Calendar.i18n(month,"mn",this)+" "+year;if(this.config&&this.config.titleHtml)
...
this.onSetTime(this.currentDateEnd);this._initMultipleDates();this.updateWCH();};Zapatec.Calendar.prototype._findDateFormMultiple=function(date,multipleRange){var d=null;for(var i in multipleRange){d=multipleRange[i];if(date.substr(0,8)==Zapatec.Date.print(d,"%Y%m%d")){this.config.multipleRange[date]=multipleRange[i];return this.config.multipleRange[date];}}
this.config.multipleRange[date]=date;return;}
Zapatec.Calendar.prototype._initMultipleDates=function(){if(this.config.multiple){var d=null;var dateEnd=null;for(var i in this.config.multiple){d=this.config.multiple[i];if(this.config.timeRange)dateEnd=this.config.multiple[i];var cell=this.datesCells[Zapatec.Date.print(d,"%Y%m%d")];if(!this.config.multiple[Zapatec.Date.print(d,"%Y%m%d")]){this.config.multiple[Zapatec.Date.print(d,"%Y%m%d")]=d;delete(this.config.multiple[i]);}
if(!d)
continue;if(cell){Zapatec.Utils.addClass(cell,"selected");}}
if(d!=null&&this.config.showsTime){this.currentDate=new Date(d);this.onSetTime();if(this.config.timeRange){this.currentDateEnd=new Date(dateEnd);this.onSetTime(this.currentDateEnd);}}}};Zapatec.Calendar.prototype._toggleMultipleDate=Zapatec.Calendar.prototype.toggleMultipleDate=function(date,date1){if(this.config.multiple){var ds=Zapatec.Date.print(date,"%Y%m%d");var cell=this.datesCells[Zapatec.Date.print(date,"%Y%m%d")];if(cell){var d=this.config.multiple[ds];if(!d){Zapatec.Utils.addClass(cell,"selected");this.config.multiple[ds]=date;if(this.config.timeRange){this.config.multipleRange[ds]=(date1?date1:date);}}else{Zapatec.Utils.removeClass(cell,"selected");delete this.config.multiple[ds];if(this.config.timeRange)delete this.config.multipleRange[ds];}}}};Zapatec.Calendar.prototype.navigateTo=function(date){if(this.setDate(date,false)&&Zapatec.Calendar.cellClick(this.currentDateEl))
...
this.tooltips.innerHTML=text;};Zapatec.Calendar.prototype.reinit=function(){this._init(this.config.firstDay,this.config.date);};Zapatec.Calendar.prototype.refresh=function(){var p=this.isPopup?null:this.element.parentNode;var x=parseInt(this.element.style.left);var y=parseInt(this.element.style.top);this.destroy();this.isCreate=false;this.init(this.config);if(this.isPopup)
this.showAt(x,y);};Zapatec.Calendar.prototype.setFirstDayOfWeek=function(firstDay){if(this.config.firstDay!=firstDay){this._init(firstDay,this.config.date);var rowsOfMonths=Math.floor(this.config.numberMonths/this.config.monthsInRow);if(this.config.numberMonths%this.config.monthsInRow>0){++rowsOfMonths;}
for(var l=1;l<=rowsOfMonths;++l){this.firstDayName=this.rowsOfDayNames[l];this._displayWeekdays();}}};Zapatec.Calendar.prototype.setDateStatusHandler=Zapatec.Calendar.prototype.setDisabledHandler=function(unaryFunction){this.getDateStatus=unaryFunction;};Zapatec.Calendar.prototype.setRange=function(A,Z){var m,a=Math.min(A,Z),z=Math.max(A,Z);this.minYear=m=Math.floor(a);this.minMonth=(m==a)?0:Math.ceil((a-m)*100-1);this.maxYear=m=Math.floor(z);this.maxMonth=(m==z)?11:Math.ceil((z-m)*100-1);};Zapatec.Calendar.prototype.setMultipleDates=function(multiple){if(!multiple||typeof multiple=="undefined")return;this.config.multiple=[];if(this.config.timeRange){var multipleRange=this.config.multipleRange;this.config.multipleRange=[];}
Zapatec.Calendar.activeCalendar=this;var ds=null;for(var i=multiple.length;--i>=0;){var d=multiple[i];ds=Zapatec.Date.print(d,"%Y%m%d");this.config.multiple[ds]=d;if(this.config.timeRange)this._findDateFormMultiple(ds,multipleRange);}};Zapatec.Calendar.prototype.selectDate=function(dates){var cal=this;function setDateRange(dateRange){var count=0;for(var i in dateRange)
...
{if(typeof this.config.flatCallback=="function"){Zapatec.Utils.sortOrder=(this.config.sortOrder!="asc"&&this.config.sortOrder!="desc"&&this.config.sortOrder!="none")?"none":this.config.sortOrder;if(this.config.multiple&&(Zapatec.Utils.sortOrder!="none")){var dateArray=new Array();for(var i in this.config.multiple){var currentDate=this.config.multiple[i];if(currentDate){dateArray[dateArray.length]=currentDate;}
dateArray.sort(Zapatec.Utils.compareDates);}
this.config.multiple={};for(var i=0;i<dateArray.length;i++){var d=dateArray[i];var ds=Zapatec.Date.print(d,"%Y%m%d");this.config.multiple[ds]=d;if(this.config.timeRange)this._findDateFormMultiple(ds);}}
this.config.flatCallback(this);}}
Zapatec.Calendar.prototype.callHandler=function(){if(this.onSelected){this.onSelected(this,Zapatec.Date.print(this.config.date,this.dateFormat));}};Zapatec.Calendar.prototype.updateHistory=function(){var a,i,d,tmp,s,str="",len=this.config.prefs.hsize-1;if(this.config.prefs.history){a=this.config.prefs.history.split(/,/);i=0;while(i<len&&(tmp=a[i++])){s=tmp.split(/\//);d=new Date(parseInt(s[0],10),parseInt(s[1],10)-1,parseInt(s[2],10),parseInt(s[3],10),parseInt(s[4],10));if(!Zapatec.Date.dateEqualsTo(d,this.config.date))
str+=","+tmp;}}
this.config.prefs.history=Zapatec.Date.print(this.config.date,"%Y/%m/%d/%H/%M")+str;Zapatec.Calendar.savePrefs(this.id);};Zapatec.Calendar.prototype.callHelpHandler=function(){var cal=this;var text=Zapatec.Calendar.i18n("ABOUT",null,cal);if(typeof text!="undefined"){text+=cal.config.showsTime?Zapatec.Calendar.i18n("ABOUT_TIME",null,cal):"";}else{text="Help and about box text is not translated into this language.\n"+"If you know this language and you feel generous please update\n"+"the corresponding file in \"lang\" subdir to match calendar-en.js\n"+"and send it back to <support@zapatec.com> to get it into the distribution  ;-)\n\n"+"Thank you!\n"+"http://www.zapatec.com\n";}
alert(text);}
...
this.create();var rows=this.table.getElementsByTagName("tr");for(var i=rows.length;i>0;){var row=rows[--i];Zapatec.Utils.removeClass(row,"rowhilite");var cells=row.getElementsByTagName("td");for(var j=cells.length;j>0;){var cell=cells[--j];Zapatec.Utils.removeClass(cell,"hilite");Zapatec.Utils.removeClass(cell,"active");}}
...
this.show();};Zapatec.Calendar.prototype.showAtElement=function(el,opts){var self=this;var p=Zapatec.Utils.getElementOffset(el);if(!opts||typeof opts!="string"){this.showAt(p.x,p.y+el.offsetHeight);return true;}
...
p.width=w;p.height=h;self.monthsCombo.style.display="none";Zapatec.Utils.fixBoxPosition(p,10);self.showAt(p.x,p.y);};Zapatec.Calendar.prototype.printWith2Time=function(date,date1,format){if(this.config.timeRange){if(!date1)date1=date;var s={};var hr=date1.getHours();var pm=(hr>=12);var ir=(pm)?(hr-12):hr;var dy=Zapatec.Date.getDayOfYear(date1);if(ir==0)
...
return Zapatec.Date.print(date,str);}
else
return Zapatec.Date.print(date,format);}
...
fmt=this.dateFormat;var date=Zapatec.Date.parseDate(str,fmt);return this.setDate(date);};Zapatec.Calendar.prototype.hideShowCovered=function(){if(!Zapatec.is_ie5)
...
if(this.config.weekNumbers&&cell){cell=cell.nextSibling;}}};Zapatec.Utils.compareDates=function(date1,date2)
{if(Zapatec.Utils.sortOrder=="asc")
...
oMsg+="unknown";oMsg+=" error";oMsg+="\n"+oError.errorDescription;alert(oMsg);};if(!Zapatec.Calendar){Zapatec.Calendar={};}
Zapatec.Calendar.Date={};Zapatec.Calendar.Date._MD=[31,28,31,30,31,30,31,31,30,31,30,31];Zapatec.Calendar.Date.SECOND=1000;Zapatec.Calendar.Date.MINUTE=60*Zapatec.Calendar.Date.SECOND;Zapatec.Calendar.Date.HOUR=60*Zapatec.Calendar.Date.MINUTE;Zapatec.Calendar.Date.DAY=24*Zapatec.Calendar.Date.HOUR;Zapatec.Calendar.Date.WEEK=7*Zapatec.Calendar.Date.DAY;Zapatec.Calendar.Date.getMonthDays=function(date,month){var year=date.getFullYear();if(typeof month=="undefined"){month=date.getMonth();}
if(((0==(year%4))&&((0!=(year%100))||(0==(year%400))))&&month==1){return 29;}else{return Zapatec.Calendar.Date._MD[month];}};Zapatec.Calendar.Date.getDayOfYear=function(date){var now=new Date(date.getFullYear(),date.getMonth(),date.getDate(),0,0,0);var then=new Date(date.getFullYear(),0,0,0,0,0);var time=now-then;return Math.round(time/Zapatec.Calendar.Date.DAY);};Zapatec.Calendar.Date.getWeekNumber=function(date){var d=new Date(date.getFullYear(),date.getMonth(),date.getDate(),0,0,0);var DoW=d.getDay();d.setDate(d.getDate()-(DoW+6)%7+3);var ms=d.valueOf();d.setMonth(0);d.setDate(4);return Math.round((ms-d.valueOf())/(7*864e5))+1;};Zapatec.Calendar.Date.equalsTo=function(date1,date2){if(!date1||!date2)
return false;return((date1.getFullYear()==date2.getFullYear())&&(date1.getMonth()==date2.getMonth())&&(date1.getDate()==date2.getDate())&&(date1.getHours()==date2.getHours())&&(date1.getMinutes()==date2.getMinutes()));};Zapatec.Calendar.Date.dateEqualsTo=function(date1,date2){if(!date1||!date2)
return false;return((date1.getFullYear()==date2.getFullYear())&&(date1.getMonth()==date2.getMonth())&&(date1.getDate()==date2.getDate()));};Zapatec.Calendar.Date.setDateOnly=function(date1,date2){var tmp=new Date(date2);date1.setDate(1);date1.setFullYear(tmp.getFullYear());date1.setMonth(tmp.getMonth());date1.setDate(tmp.getDate());};Zapatec.Calendar.Date.print=function(date,str){var m=date.getMonth();var d=date.getDate();var y=date.getFullYear();var wn=/%U|%W|%V/.test(str)?Zapatec.Calendar.Date.getWeekNumber(date):"";var w=date.getDay();var s={};var hr=date.getHours();var pm=(hr>=12);var ir=(pm)?(hr-12):hr;var dy=/%j/.test(str)?Zapatec.Calendar.Date.getDayOfYear(date):"";if(ir==0)
ir=12;var min=date.getMinutes();var sec=date.getSeconds();if(typeof Zapatec.Calendar.i18n=='function'){s["%a"]=/%a/.test(str)?Zapatec.Calendar.i18n(w,"sdn"):"";s["%A"]=/%A/.test(str)?Zapatec.Calendar.i18n(w,"dn"):"";s["%b"]=/%b/.test(str)?Zapatec.Calendar.i18n(m,"smn"):"";s["%B"]=/%B/.test(str)?Zapatec.Calendar.i18n(m,"mn"):"";}
s["%C"]=1+Math.floor(y/100);s["%d"]=(d<10)?("0"+d):d;s["%e"]=d;s["%H"]=(hr<10)?("0"+hr):hr;s["%I"]=(ir<10)?("0"+ir):ir;s["%j"]=(dy<100)?((dy<10)?("00"+dy):("0"+dy)):dy;s["%k"]=hr?hr:"0";s["%l"]=ir;s["%m"]=(m<9)?("0"+(1+m)):(1+m);s["%M"]=(min<10)?("0"+min):min;s["%n"]="\n";if(typeof Zapatec.Calendar.i18n=='function'){s["%P"]=s["%p"]=pm?Zapatec.Calendar.i18n("pm","AMPM"):Zapatec.Calendar.i18n("am","AMPM");}else{s["%P"]=s["%p"]=pm?"pm":"am";}
s["%s"]=Math.floor(date.getTime()/1000);s["%S"]=(sec<10)?("0"+sec):sec;s["%t"]="\t";s["%U"]=s["%W"]=s["%V"]=(wn<10)?("0"+wn):wn;s["%u"]=(w==0)?7:w;s["%w"]=w?w:"0";s["%y"]=''+y%100;if(s["%y"]<10){s["%y"]="0"+s["%y"];}
...
return str;};Zapatec.Calendar.Date.parseDate=function(str,format){var fmt=format,strPointer=0,token=null,parseFunc=null,valueLength=null,valueRange=null,valueType=null,date=new Date(),values={};var numberRules=["%d","%H","%I","%m","%M","%S","%s","%W","%u","%w","%y","%e","%k","%l","%s","%Y","%C","%z","%Z"];function isNumberRule(rule){if(Zapatec.Utils.arrIndexOf(numberRules,rule)!=-1){return true;}
return false;}
...
if(wasParsed(values["%W"])){var weekNumber=Zapatec.Calendar.Date.getWeekNumber(date);if(weekNumber!=values["%W"]){date.setDate(date.getDate()+(values["%W"]-weekNumber)*7);}}
...
if(wasParsed(values["%Z"])){date.setUTCHours(date.getUTCHours()+(date.getTimezoneOffset()/60+parseInt(values["%Z"],10)));date.setUTCMinutes(date.getUTCMinutes()+(-1)*(date.getTimezoneOffset()%60-parseInt(values["%Zm"],10)))}}
if(Zapatec.Calendar.Date.print(date,format)!=str){return null;}
return date;};Zapatec.Calendar.Date.setFullYear=function(date,y){var d=new Date(date);d.setFullYear(y);if(d.getMonth()!=date.getMonth())
date.setDate(28);date.setFullYear(y);};Zapatec.Calendar.Date.compareDatesOnly=function(date1,date2){var year1=date1.getYear();var year2=date2.getYear();var month1=date1.getMonth();var month2=date2.getMonth();var day1=date1.getDate();var day2=date2.getDate();if(year1>year2){return-1;}
...
return 0;}
function inheritProperties(oSubClass,oSuperClass){for(var i in oSuperClass){oSubClass[i]=oSuperClass[i];}}
Zapatec.Date={};inheritProperties(Zapatec.Date,Zapatec.Calendar.Date);Zapatec.Calendar.setup=function(oArg){return new Zapatec.Calendar(oArg);};Zapatec.Calendar.setup.id="Zapatec.Calendar.setup";Zapatec.inherit(Zapatec.Calendar.setup,Zapatec.Calendar);Zapatec.Utils.createNestedHash(Zapatec,["Langs","Zapatec.Calendar","en"],{"_DN":new Array
