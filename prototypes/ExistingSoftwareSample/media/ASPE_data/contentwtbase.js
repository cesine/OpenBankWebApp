...
function dcsFPC(offset){
	...
	document.cookie=name+"="+"id="+WT.co_f+":lv="+dCur.getTime().toString()+":ss="+dSes.getTime().toString()+expiry+"; path=/"+(((dcsBase.fpcdom!=""))?("; domain="+dcsBase.fpcdom):(""));
	if (document.cookie.indexOf(name+"=")==-1){
...
//	WT.sp="@@SPLITVALUE@@";
	...
	if (window.location.search){
		...
		if (dcsBase.qp.length>0){
			for (var i=0;i<dcsBase.qp.length;i++){
				var pos=DCS.dcsqry.indexOf(dcsBase.qp[i]);
				if (pos!=-1){
					var front=DCS.dcsqry.substring(0,pos);
					var end=DCS.dcsqry.substring(pos+dcsBase.qp[i].length,DCS.dcsqry.length);
...
function dcsA(N,V){
	if (dcsBase.i18n&&(dcsBase.exre!="")&&!dcsBase.exre.test(N)){
		if (N=="dcsqry"){
			var newV="";
	...
	return "&"+N+"="+dcsEscape(V, dcsBase.re);
}
...
function dcsCreateImage(dcsSrc){
	if (document.images){
		dcsBase.images[dcsBase.index]=new Image;
		dcsBase.images[dcsBase.index].src=dcsSrc;
		dcsBase.index++;
	}
	else{
...
function dcsMeta(){
	...
	if (typeof(elems)!="undefined"){
		...
		for (var i=0;i<length;i++){
			...
			else if (dcsBase.i18n&&(equiv=="Content-Type")){
...
function dcsTag(){
	...
	var P="https://"+dcsInit.domain+(dcsInit.dcsid==""?'':'/'+dcsInit.dcsid)+"/dcs.gif?";
	for (var N in DCS){
		if (DCS[N]){
			P+=dcsA(N,DCS[N]);
		}
