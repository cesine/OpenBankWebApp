...
var val=Zapatec.Form.Utils.getValue(formEl);if(val&&val instanceof Array){for(var ii=0;ii<val.length;ii++){arrContent.push(formEl.name+'='+escape(val[ii]));}}else{arrContent.push(formEl.name+'='+escape(val));}}
...
//wlew Jan 14 2009 (Red star icon showed up besides the disabled fields even if the field already have correct value)
//https://www.zapatec.com/website/forums/viewtopic.php?t=1039)
...
{
	this.statusImg.className=Zapatec.Form.IGNORE_CLASSNAME+ ' zpNotRequired';
}
else
{
	this.statusImg.className=Zapatec.Form.IGNORE_CLASSNAME+ (this.hasFeature("zpFormRequired")?' zpIsRequired':' zpNotRequired');
}
this.statusImg.id="zpFormField"+this.id+"StatusImg";this.addCircularRef(this,"statusImg");this.addCircularRef(this,"fetchingMark");//this.addCircularRef(this,"validMark");
this.addCircularRef(this,"emptyMark");this.addCircularRef(this,"editingMark");this.addCircularRef(this,"requiredMark");this.errorText=Zapatec.Utils.createElement('span');this.errorText.id="zpFormField"+this.id+"ErrorText";this.errorText.className=Zapatec.Form.IGNORE_CLASSNAME+' zpFormError';if(this.field.type&&this.field.type.toLowerCase()=="hidden"){this.errorText.style.display='none';this.requiredMark.style.display='none';}
//end of change
...
//WL Jan 14 2009 - Upgrade Zapatec, change date format
//Zapatec.Form.Validator.isDateValid=function(str,fmt){if(fmt==null||fmt==""){fmt="%m/%d/%y";}
Zapatec.Form.Validator.isDateValid=function(str,fmt){if(fmt==null||fmt==""){fmt="%d/%m/%y";}
