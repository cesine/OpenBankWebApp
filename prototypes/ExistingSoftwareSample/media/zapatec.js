...
return;}}};if(typeof Zapatec=='undefined'){Zapatec=function(){};}
...
if(bOnetime){oEvent.listeners.push({listener:fListener,onetime:true});}else{oEvent.listeners.push(fListener);}};Zapatec.EventDriven.addOnetimeEventListener=function(sEvent,fListener){return Zapatec.EventDriven.addEventListener(sEvent,fListener,true);};Zapatec.EventDriven.unshiftEventListener=function(sEvent,fListener){if(typeof fListener!='function'){return false;}
var oEvents=zapatecEventDriven.events;var oEvent=oEvents[sEvent];if(!oEvent){oEvents[sEvent]={listeners:[]};oEvent=oEvents[sEvent];}else{zapatecEventDriven.removeEventListener(sEvent,fListener);}
...
Zapatec.EventDriven.removeOnetimeEventListeners(sEvent);};Zapatec.ImagePreloader=function(objArgs){this.job=null;this.image=null;if(arguments.length>0)this.init(objArgs);};Zapatec.ImagePreloader.prototype.init=function(objArgs){if(!objArgs||!objArgs.job){return;}
...
if(typeof Zapatec=='undefined'){Zapatec=function(){};}
Zapatec.Transport=function(){};zapatecTransport=Zapatec.Transport;if(typeof ActiveXObject!='undefined'){Zapatec.Transport.XMLDOM=null;Zapatec.Transport.XMLHTTP=null;Zapatec.Transport.pickActiveXVersion=function(aVersions){for(var iVn=0;iVn<aVersions.length;iVn++){try{var oDoc=new ActiveXObject(aVersions[iVn]);if(oDoc){return aVersions[iVn];}}catch(oExpn){};}
...
Zapatec.Transport.showBusy(oArg);var bErrorDisplayed=false;var funcOnReady=function(){Zapatec.Transport.removeBusy(oArg);try{if(oRequest.status==200||oRequest.status==304||(location.protocol=='file:'&&!oRequest.status)){if(typeof oArg.onLoad=='function'){try{oArg.onLoad(oRequest);}catch(oExpn){Zapatec.Transport.displayError(0,"Error: Can't process result from "+oArg.url+'.\n'+
(oExpn.message||''),oArg.onError);}}}else if(!bErrorDisplayed){bErrorDisplayed=true;Zapatec.Transport.displayError(oRequest.status,"Error: Can't fetch "+oArg.url+'.\n'+
...
(oExpn.message||''),oArg.onError);}}};return null;};Zapatec.Transport.parseHtml=function(sHtml,oContainer){sHtml+='';sHtml=sHtml.replace(/^\s+/g,'');var oTmpContr;if(document.createElementNS){oTmpContr=document.createElementNS('http://www.w3.org/1999/xhtml','div');}else{oTmpContr=document.createElement('div');}
...
//wlew Jan 14 2009, Change from previosu version (This block in the new version cause the ing.css images not to show in IE. After investigating, Parviz realized that IE puts the path to the images 2 times)
...
//if(sUrl.charAt(0)=='/'){return[oLocation.protocol,'//',oLocation.hostname,sPort,sUrl].join('');}
...
Zapatec.Drag=function(){};zapatecDrag=Zapatec.Drag;zapatecUtils.emulateWindowEvent(['mousedown','mousemove','mouseup']);Zapatec.Drag.currentId=null;Zapatec.Drag.start=function(oEv,sId,oArg){var oEl=Zapatec.Widget.getElementById(sId);if(!oEl||oEl.zpDragArgs){return true;}
if(!oArg){oArg={};}
...
Zapatec.Widget=function(oArg){this.arg=oArg;this.config={};Zapatec.Widget.SUPERconstructor.call(this);this.init(oArg);};Zapatec.inherit(Zapatec.Widget,Zapatec.EventDriven);zapatecWidget=Zapatec.Widget;Zapatec.Widget.id='Zapatec.Widget';Zapatec.Widget.path=Zapatec.getPath('Zapatec.Widget');Zapatec.Widget.prototype.init=function(oArg){Zapatec.Widget.SUPERclass.init.call(this);if(typeof this.id=='undefined'){var iId=0;while(Zapatec.Widget.all[iId]){iId++;}
...
if(this.config.lang&&this.config.lang.length>0&&!(Zapatec.Langs[this.config.langId]&&Zapatec.Langs[this.config.langId][this.langStr])){Zapatec.Log({description:this.config.lang+(this.config.langCountryCode?" and country code "+this.config.langCountryCode:"")+(this.config.langEncoding?" and encoding "+this.config.langEncoding:"")});this.config.lang=null;this.config.langEncoding=null;this.langStr=null;}};Zapatec.Widget.prototype.reset=function(){this.config={};this.reconfigure(this.arg);};Zapatec.Widget.prototype.configure=function(oArg){this.defineConfigOption('theme','default');this.defineConfigOption('templateFile','default');var sPath=this.constructor.path;if(typeof sPath!='undefined'){this.defineConfigOption('themePath',Zapatec.Transport.translateUrl({url:'../themes/',relativeTo:sPath}));this.defineConfigOption('templateFilePath',Zapatec.Transport.translateUrl({url:'../templates/',relativeTo:sPath}));}else{this.defineConfigOption('templateFilePath','../templates/');}
this.defineConfigOption('templates');this.defineConfigOption('asyncTheme',false);this.defineConfigOption('source');this.defineConfigOption('sourceType','html');this.defineConfigOption('sourceFetchMethod','GET');this.defineConfigOption('sourceFetchContentType');this.defineConfigOption('sourceFetchContent');this.defineConfigOption('callbackSource');this.defineConfigOption('asyncSource',true);this.defineConfigOption('reliableSource',true);this.defineConfigOption('callbackConvertSource');this.defineConfigOption('eventListeners',{});this.defineConfigOption('langId');this.defineConfigOption('lang');this.defineConfigOption('langCountryCode');this.defineConfigOption('langEncoding');var oConfig=this.config;var sOption;if(oArg){for(sOption in oArg){if(typeof oConfig[sOption]!='undefined'){oConfig[sOption]=oArg[sOption];}else if(Zapatec.Debug){Zapatec.Debug.log.warn('Unknown config option: '+sOption+'.');}}}
...
for(var iRef=this.widgetCircularRefs.length-1;iRef>=0;iRef--){var oRef=this.widgetCircularRefs[iRef];oRef[0][oRef[1]]=null;oRef[0]=null;}};Zapatec.Widget.prototype.discard=function(){this.discardEventHandlers();this.discardControls();this.discardTemplates();this.arg=null;this.config=null;Zapatec.Widget.all[this.id]=null;this.removeCircularRefs();};Zapatec.Widget.removeCircularRefs=function(){var aWidgets=zapatecWidget.all;var oWidget;for(var iWidget=aWidgets.length-1;iWidget>=0;iWidget--){var oWidget=aWidgets[iWidget];if(oWidget&&typeof oWidget.discard=='function'){oWidget.discard();}}};zapatecUtils.addEvent(window,'unload',zapatecWidget.removeCircularRefs);Zapatec.Widget.prototype.defineConfigOption=function(sOption,val){if(typeof this.config[sOption]=='undefined'){if(typeof val=='undefined'){this.config[sOption]=null;}else{this.config[sOption]=val;}}};Zapatec.Widget.prototype.addUserEventListeners=function(){var oListeners=this.config.eventListeners;var fListener,iListeners,iListener;for(var sEvent in oListeners){if(oListeners.hasOwnProperty(sEvent)){vListener=oListeners[sEvent];if(vListener instanceof Array){iListeners=vListener.length;for(iListener=0;iListener<iListeners;iListener++){this.addEventListener(sEvent,vListener[iListener]);}}else{this.addEventListener(sEvent,vListener);}}}};Zapatec.Widget.prototype.addStandardEventListeners=function(){this.addEventListener('fetchSourceError',Zapatec.Widget.loadSourceError);this.addEventListener('loadThemeError',Zapatec.Widget.loadThemeError);this.addEventListener('loadTemplateError',Zapatec.Widget.loadTemplateError);};Zapatec.Widget.loadThemeError=function(oError){if(Zapatec.Debug){var sDescription='';if(oError&&oError.errorDescription){sDescription=oError.errorDescription;}
Zapatec.Debug.log.error("Can't load theme: "+sDescription+'.');}};Zapatec.Widget.loadTemplateError=function(oError){if(Zapatec.Debug){var sDescription='';if(oError&&oError.errorDescription){sDescription=oError.errorDescription;}
Zapatec.Debug.log.error("Can't load template: "+sDescription+'.');}};Zapatec.Widget.loadSourceError=function(oError){if(Zapatec.Debug){var sDescription='';if(oError&&oError.errorDescription){sDescription=oError.errorDescription;}
Zapatec.Debug.log.error("Can't load source: "+sDescription+'.');}};Zapatec.Widget.prototype.loadTheme=function(){var oConfig=this.config;if(typeof oConfig.theme=='string'&&oConfig.theme.length){var iPos=oConfig.theme.lastIndexOf('/');if(iPos>=0){iPos++;oConfig.themePath=oConfig.theme.substring(0,iPos);oConfig.theme=oConfig.theme.substring(iPos);}
...
if(oConfig.theme){this.fireEvent('loadThemeStart');this.themeLoaded=false;var oWidget=this;Zapatec.Transport.loadCss({url:oConfig.themePath+oConfig.theme+'.css',async:oConfig.asyncTheme,onLoad:function(){if(!oWidget){return;}
oWidget.fireEvent('loadThemeEnd');oWidget.themeLoaded=true;},onError:function(oError){if(!oWidget){return;}
oWidget.fireEvent('loadThemeEnd');oWidget.fireEvent('loadThemeError',oError);oWidget.themeLoaded=true;}});}};Zapatec.Widget.prototype.loadTemplate=function(){var oConfig=this.config;if(typeof oConfig.templateFile=='string'&&oConfig.templateFile.length){var iPos=oConfig.templateFile.lastIndexOf('/');if(iPos>=0){iPos++;oConfig.templateFilePath=oConfig.templateFile.substring(0,iPos);oConfig.templateFile=oConfig.templateFile.substring(iPos);}
...
if(oConfig.templateFile&&oConfig.templates){this.fireEvent('loadTemplateStart');var oWidget=this;Zapatec.Transport.fetch({url:oConfig.templateFilePath+oConfig.templateFile+'.html?'+
Math.random(),onLoad:function(oRequest){if(!oWidget){return;}
oWidget.fireEvent('loadTemplateEnd');var oContainer=zapatecTransport.parseHtml(oRequest.responseText);oContainer.style.display='none';document.body.insertBefore(oContainer,document.body.firstChild);oWidget.initTemplates();document.body.removeChild(oContainer);oContainer=null;oWidget.display();oWidget=null;},onError:function(oError){oWidget.fireEvent('loadTemplateError',oError);oWidget.fireEvent('loadTemplateEnd');}});}};Zapatec.Widget.prototype.initTemplates=function(){this.discardControls();this.discardTemplates();this.templates={};var oTemplates=this.templates;var oTemplateContainers=this.config.templates;if(!oTemplateContainers){if(typeof zapatecDebug=='function'){zapatecDebug.log.error('Missing config option "templates".');}
return;}
if(!zapatecTemplate){if(typeof zapatecDebug=='function'){zapatecDebug.log.error('Zapatec.Template class was not found.');}
return;}
var sTplContainer,sTplContainerId;for(sTplContainer in oTemplateContainers){sTplContainerId=oTemplateContainers[sTplContainer];if(typeof sTplContainerId=='string'&&sTplContainerId.length){oTemplates[sTplContainer]=new zapatecTemplate({source:sTplContainerId,sourceType:'html'});}}};Zapatec.Widget.prototype.discardTemplates=function(){var oTemplates=this.templates;if(oTemplates){var sTpl,oTpl;for(sTpl in oTemplates){oTpl=oTemplates[sTpl];if(oTpl&&typeof oTpl.discard=='function'){oTpl.discard();oTemplates[sTpl]=null;}}}};Zapatec.Widget.prototype.display=function(){if(!this.templates){if(typeof zapatecDebug=='function'){zapatecDebug.log.error('Templates are not loaded.');}}};Zapatec.Widget.prototype.initControls=function(){this.discardControls();this.controls={};};Zapatec.Widget.prototype.discardControls=function(){var oControls=this.controls;if(oControls){var sControl,oControl;for(sControl in oControls){oControl=oControls[sControl];if(oControl&&typeof oControl.discard=='function'){oControl.discard();oControls[sControl]=null;}}}};Zapatec.Widget.prototype.setEventHandlers=function(){this.discardEventHandlers();this.eventHandlers={};};Zapatec.Widget.prototype.discardEventHandlers=function(){var oHandlers=this.eventHandlers;if(oHandlers){var sHandler,oHandler,sEvent,fHandler;for(sHandler in oHandlers){oHandler=oHandlers[sHandler];if(oHandler){oEl=oHandler.element;sEvent=oHandler.event;fHandler=oHandler.handler;if(oEl&&typeof sEvent=='string'&&typeof fHandler=='function'){Zapatec.Utils.removeEvent(oEl,sEvent,fHandler);oHandler.element=null;oHandler.handler=null;oHandlers[sHandler]=null;}}}}};Zapatec.Widget.prototype.getClassName=function(oArg){var aClassName=[];if(oArg&&oArg.prefix){aClassName.push(oArg.prefix);}
...
//wlew Jan 14 2009, Added to make sure path is OK
Zapatec.include('/zapatec/zpform/src/form.js', 'Zapatec.FormWidget');Zapatec.include('/zapatec/zpform/src/ing-validator.js', 'Zapatec.FormWidget');