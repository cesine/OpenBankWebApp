...
//wlew Jan 14 2009, Copy from previous ING version to add bottom of tooltip
...
Zapatec.Utils.addClass(this.tooltip,this.getClassName({prefix:"zpTooltip",suffix:""}));Zapatec.Transport.showBusy({busyContainer:this.tooltip});this.loadData();this.saveCookie();Zapatec.Transport.removeBusy({busyContainer:this.tooltip});
if(this.tooltip.title){var title=Zapatec.Utils.createElement("div");this.tooltip.insertBefore(title,this.tooltip.firstChild);title.className=this.getClassName({prefix:"zpTooltip",suffix:"Title"});title.innerHTML=unescape(this.tooltip.title);this.tooltip.title="";
	var bottom=Zapatec.Utils.createElement("div");
	Zapatec.Utils.insertAfter(this.tooltip.lastChild,bottom);
	bottom.className=this.getClassName({prefix:"zpTooltip",suffix:"Bottom"});
}
//end of change
...
//wlew Jan 14 2009 - Copy from previous ING version
...
//div.appendChild(dfn.firstChild);dfn.innerHTML="?";oTooltip=new Zapatec.Tooltip({target:dfn,parent:document.body,source:div});dfnClass=oTooltip.getClassName({prefix:"zpTooltip",suffix:"Dfn"});Zapatec.Utils.addClass(dfn,dfnClass);}}};

Zapatec.Tooltip.setupFromDFN=function(dfnTheme, dfnOffset){
	var dfns=document.getElementsByTagName("dfn");
	for(var i=0;i<dfns.length;++i){
		var dfn=dfns[i];
		var div=document.createElement("div");
		if(dfn.title){
			div.title=dfn.title;dfn.title="";
		}
		while(dfn.firstChild){
			div.appendChild(dfn.firstChild);
		}
		dfn.innerHTML="&nbsp;";
		var oTooltip=new Zapatec.Tooltip({target:dfn,parent:document.body,tooltip:div,offsetX:dfnOffset,theme:dfnTheme});
		dfn.className=oTooltip.getClassName({prefix:"zpTooltip",suffix:"Dfn"});
	}
};
//end of change

Zapatec.Tooltip._currentTooltip=null;
Zapatec.Tooltip.prototype._onMouseMove=function(ev){ev||(ev=window.event);if(!this.isCreate)
...
//Sep 11 2009, Patch to fix tooltip on IE 8
